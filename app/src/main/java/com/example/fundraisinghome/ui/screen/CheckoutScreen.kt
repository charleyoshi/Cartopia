                ...
                Button(
                    onClick = {
                        ...
                        coroutineScope.launch {
                            ...
                            val shippingDetails = ShippingDetails_(
                                name = name.value,
                                address = address.value,
                                city = city.value,
                                postalCode = postalCode.value
                            )
                            val shippingDetailsId =
                                shippingDetailsRepository.insertShippingDetails(shippingDetails).toString()


                            val paymentDetails = PaymentDetails_(
                            ...
                            val paymentDetailsId = paymentDetailsRepository.insertPaymentDetails(paymentDetails).toString()

                            // Create Order object
                            val order = Order_(
                                orderId = UUID.randomUUID().toString(), // Generate unique order ID
                                userId = UserSingleton.getCurrentUser()?.id.toString(),
                            ...
                            // Insert OrderItems into database
                            cartUiState.items.forEach { cartItem ->
                                val orderItem = OrderItem(orderId =  order.orderId, productId =  cartItem.productId, quantity =  cartItem.quantity)
